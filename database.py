...
class Database:
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute('''
                CREATE TABLE IF NOT EXISTS jackpot (
                    jp_id INT PRIMARY KEY DEFAULT 1,
            ...
            await conn.execute('''
                CREATE TABLE IF NOT EXISTS daily_tasks (
                    ...
                    reward_vc BIGINT NOT NULL,
                    reward_vt DECIMAL(10,2) NOT NULL,
    ...
    async def update_vt_balance(self, user_id: int, amount: float, add: bool = True):
        async with self.pool.acquire() as conn:
            if add:
                await conn.execute(
                    'UPDATE users SET vt_balance = vt_balance + $1 WHERE user_id = $2',
                    amount, user_id)
            else:
                await conn.execute(
                    'UPDATE users SET vt_balance = GREATEST(vt_balance - $1, 0) WHERE user_id = $2',
                    amount, user_id)
    ...
    async def update_bank(self, user_id: int, amount: int, add: bool = True):
        async with self.pool.acquire() as conn:
            if add:
                ...
                await conn.execute(
                    'UPDATE users SET bank_balance = GREATEST(bank_balance - $1, 0) WHERE user_id = $2',
                    amount, user_id)
    ...
    async def get_top_by_balance(self, limit: int = 10):
        async with self.pool.acquire() as conn:
            return await conn.fetch(
                'SELECT * FROM users WHERE is_banned = FALSE ORDER BY balance DESC LIMIT $1', limit)

    async def get_top_by_vt(self, limit: int = 10):
        async with self.pool.acquire() as conn:
            return await conn.fetch(
                'SELECT * FROM users WHERE is_banned = FALSE ORDER BY vt_balance DESC LIMIT $1', limit)
    ...
    async def get_user_position_in_top(self, user_id: int, by_field: str = 'balance') -> int:
        if by_field not in ('balance', 'vt_balance', 'xp', 'games_won'):
            by_field = 'balance'
        async with self.pool.acquire() as conn:
            result = await conn.fetchval(f'''
                SELECT COUNT(*) + 1 FROM users
                WHERE {by_field} > (SELECT COALESCE({by_field}, 0) FROM users WHERE user_id = $1)
                AND is_banned = FALSE
            ''', user_id)
            return result or 1
    ...
    async def get_user_gpu(self, user_id: int, gpu_type: str):
        async with self.pool.acquire() as conn:
            return await conn.fetchrow(
                'SELECT * FROM user_gpus WHERE user_id = $1 AND gpu_type = $2', user_id, gpu_type)
        ...
        async with self.pool.acquire() as conn:
            ...
            for gpu in gpus:
                ...
                if config and gpu['count'] > 0:
                    ...
                    total_vt += hours * config['vt_per_hour'] * gpu['count']
            if total_vt > 0:
                await self.update_vt_balance(user_id, total_vt, add=True)
                await conn.execute('UPDATE user_gpus SET last_collect = NOW() WHERE user_id = $1', user_id)
            return total_vt
    ...
    async def get_vt_price(self):
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow('SELECT price FROM vt_price ORDER BY vtp_id DESC LIMIT 1')
        ...
        async with self.pool.acquire() as conn:
            if user_id:
                ...
                return await conn.fetch(
                    ...
                    order_type)
            return await conn.fetch(
                'SELECT * FROM market_orders WHERE is_active = TRUE ORDER BY created_at DESC LIMIT 20')

    async def cancel_order(self, order_id: int, user_id: int):
        async with self.pool.acquire() as conn:
            order = await conn.fetchrow(
                'SELECT * FROM market_orders WHERE order_id = $1 AND user_id = $2 AND is_active = TRUE',
                order_id, user_id)
            if order:
                await conn.execute('UPDATE market_orders SET is_active = FALSE WHERE order_id = $1', order_id)
    ...
    async def execute_order(self, order_id: int, buyer_id: int):
        async with self.pool.acquire() as conn:
            order = await conn.fetchrow(
                'SELECT * FROM market_orders WHERE order_id = $1 AND is_active = TRUE', order_id)
            if order:
                await conn.execute('UPDATE market_orders SET is_active = FALSE WHERE order_id = $1', order_id)
                return order
            return None
    ...
    async def use_promocode(self, user_id: int, code: str):
        async with self.pool.acquire() as conn:
            promo = await conn.fetchrow('''
                ...
                WHERE UPPER(code) = UPPER($1) AND expires_at > NOW() AND current_uses < max_uses
            ''', code)
            if not promo:
            ...
            await conn.execute(
                ...
                user_id, promo['promo_id'])
            await conn.execute(
                'UPDATE promocodes SET current_uses = current_uses + 1 WHERE promo_id = $1',
                promo['promo_id'])
    ...
    async def get_jackpot(self):
        async with self.pool.acquire() as conn:
            return await conn.fetchrow('SELECT * FROM jackpot WHERE jp_id = 1')

    async def add_to_jackpot(self, amount: int):
        contribution = max(int(amount * 0.0001), 1)
        async with self.pool.acquire() as conn:
            await conn.execute('UPDATE jackpot SET amount = amount + $1 WHERE jp_id = 1', contribution)
    ...
    async def get_jackpot_participants_count(self):
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                'SELECT COUNT(*) FROM users WHERE jackpot_registered = TRUE AND is_banned = FALSE')
    ...
    async def draw_jackpot(self):
        async with self.pool.acquire() as conn:
            ...
            await conn.execute('''
                UPDATE jackpot SET amount = 1000000, last_winner = $1, last_draw = NOW() WHERE jp_id = 1
            ''', winner['user_id'])
    ...
    async def get_president(self):
        async with self.pool.acquire() as conn:
            return await conn.fetchrow('''
                ...
                FROM president p JOIN users u ON p.user_id = u.user_id
                ORDER BY p.elected_at DESC LIMIT 1
            ''')

    async def add_president_profit(self, amount: int):
        profit = max(int(amount * 0.0001), 1)
        async with self.pool.acquire() as conn:
            president = await self.get_president()
            if president:
                await conn.execute(
                    'UPDATE president SET profit = profit + $1 WHERE pres_id = $2',
                    profit, president['pres_id'])
                await self.update_balance(president['user_id'], profit, add=True)

    async def place_president_bet(self, user_id: int, amount: int):
        async with self.pool.acquire() as conn:
            existing = await conn.fetchrow(
                'SELECT * FROM president_bets WHERE user_id = $1 AND bet_date = CURRENT_DATE', user_id)
            if existing:
                await conn.execute('''
                ...
                await conn.execute(
                    'INSERT INTO president_bets (user_id, bet_amount) VALUES ($1, $2)',
                    user_id, amount)
    ...
    async def get_president_bets(self):
        async with self.pool.acquire() as conn:
            return await conn.fetch('''
                ...
                FROM president_bets pb JOIN users u ON pb.user_id = u.user_id
                WHERE pb.bet_date = CURRENT_DATE ORDER BY pb.bet_amount DESC
            ''')
    ...
    async def elect_president(self):
        async with self.pool.acquire() as conn:
            ...
            eligible = [b for b in bets if not current_president or b['user_id'] != current_president['user_id']]
            if not eligible:
                return None
            ...
            winner = eligible[-1]
            for bet in eligible:
                cumul += bet['bet_amount']
                if rand <= cumul:
                    winner = bet
                    break
            for bet in eligible:
                if bet['user_id'] != winner['user_id']:
                    await self.update_balance(bet['user_id'], bet['bet_amount'] // 2, add=True)
            if current_president:
                await conn.execute('UPDATE users SET is_president = FALSE WHERE user_id = $1',
                                   current_president['user_id'])
            await conn.execute('INSERT INTO president (user_id, profit) VALUES ($1, 0)', winner['user_id'])
    ...
    async def add_business(self, name: str, base_price: int, base_profit: int, emoji: str = '🏢'):
        async with self.pool.acquire() as conn:
            await conn.execute('''
                INSERT INTO businesses (name, base_price, base_profit, emoji) VALUES ($1, $2, $3, $4)
    ...
    async def get_business(self, business_id: int):
        async with self.pool.acquire() as conn:
            return await conn.fetchrow('SELECT * FROM businesses WHERE biz_id = $1', business_id)

    async def get_user_businesses(self, user_id: int):
        async with self.pool.acquire() as conn:
            return await conn.fetch('''
                SELECT ub.*, b.name, b.base_price, b.base_profit, b.emoji
                FROM user_businesses ub JOIN businesses b ON ub.business_id = b.biz_id
                WHERE ub.user_id = $1
            ''', user_id)
    ...
    async def get_user_business(self, user_id: int, user_business_id: int):
        async with self.pool.acquire() as conn:
            return await conn.fetchrow('''
                SELECT ub.*, b.name, b.base_price, b.base_profit, b.emoji
                FROM user_businesses ub JOIN businesses b ON ub.business_id = b.biz_id
                WHERE ub.ub_id = $1 AND ub.user_id = $2
            ''', user_business_id, user_id)

    async def buy_business(self, user_id: int, business_id: int):
        async with self.pool.acquire() as conn:
            business = await conn.fetchrow('SELECT * FROM businesses WHERE biz_id = $1', business_id)
            if not business:
                return None, "Бизнес не найден"
            user = await self.get_user(user_id)
            user_businesses = await self.get_user_businesses(user_id)
            if any(ub['business_id'] == business_id for ub in user_businesses):
    ...
    async def collect_business_profit(self, user_id: int, user_business_id: int):
        async with self.pool.acquire() as conn:
            ...
            if profit > 0:
                ...
                await conn.execute('UPDATE user_businesses SET last_collect = NOW() WHERE ub_id = $1',
                                   user_business_id)
            return profit
    ...
    async def get_pending_business_profit(self, user_id: int, user_business_id: int):
        async with self.pool.acquire() as conn:
            ...
            if not ub:
                return 0
            hours = (datetime.now() - ub['last_collect']).total_seconds() / 3600
            return int(hours * ub['base_profit'] * float(ub['profit_multiplier']))

    async def upgrade_business(self, user_id: int, user_business_id: int, cost: int):
        async with self.pool.acquire() as conn:
            ub = await conn.fetchrow(
                'SELECT * FROM user_businesses WHERE ub_id = $1 AND user_id = $2',
                user_business_id, user_id)
            if not ub:
                return False
            user = await self.get_user(user_id)
            ...
            new_mult = float(ub['profit_multiplier']) + 0.1
            await conn.execute('''
                UPDATE user_businesses SET profit_multiplier = $1, upgrade_level = upgrade_level + 1
                WHERE ub_id = $2
            ''', new_mult, user_business_id)
            return True
    ...
    async def sell_business(self, user_id: int, user_business_id: int):
        async with self.pool.acquire() as conn:
            ...
            await conn.execute('DELETE FROM user_businesses WHERE ub_id = $1', user_business_id)
            return sell_price
    ...
    async def pay_business_tax(self, user_id: int, user_business_id: int):
        async with self.pool.acquire() as conn:
            ...
            await conn.execute('''
                UPDATE user_businesses SET tax_paid_until = tax_paid_until + INTERVAL '1 day'
                WHERE ub_id = $1
            ''', user_business_id)
            return tax, None
    ...
    async def add_business_slot(self, user_id: int):
        async with self.pool.acquire() as conn:
            await conn.execute(
                'UPDATE users SET business_slots = business_slots + 1 WHERE user_id = $1', user_id)
    ...
    async def get_user_tasks(self, user_id: int):
        async with self.pool.acquire() as conn:
            return await conn.fetch(
                'SELECT * FROM daily_tasks WHERE user_id = $1 AND task_date = CURRENT_DATE', user_id)
    ...
    async def generate_daily_tasks(self, user_id: int):
        async with self.pool.acquire() as conn:
            ...
            if existing:
                return existing
            task_types = [
                'labyrinth_play', 'higher_lower_win', 'knb_play', 'crash_win',
                'mines_play', 'diamonds_play', 'roulette_play', 'blackjack_play',
            ]
    ...
    async def update_task_progress(self, user_id: int, task_type: str, increment: int = 1):
        async with self.pool.acquire() as conn:
            ...
            await conn.execute('UPDATE daily_tasks SET task_progress = $1 WHERE dt_id = $2',
                               new_progress, task['dt_id'])
            if new_progress >= task['task_target']:
                await conn.execute('UPDATE daily_tasks SET is_completed = TRUE WHERE dt_id = $1', task['dt_id'])
    ...
    async def start_game(self, user_id: int, game_type: str, bet: int, game_data: dict):
        async with self.pool.acquire() as conn:
            await conn.execute(
                'DELETE FROM active_games WHERE user_id = $1 AND game_type = $2', user_id, game_type)
            await conn.execute('''
    ...
    async def get_active_game(self, user_id: int, game_type: str):
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                'SELECT * FROM active_games WHERE user_id = $1 AND game_type = $2', user_id, game_type)
            if row:
                data = dict(row)
                gd = data['game_data']
                if isinstance(gd, str):
                    data['game_data'] = json.loads(gd)
                return data
            return None
    ...
    async def update_game(self, user_id: int, game_type: str, game_data: dict):
        async with self.pool.acquire() as conn:
            await conn.execute('''
                UPDATE active_games SET game_data = $1 WHERE user_id = $2 AND game_type = $3
    ...
    async def create_daily_bonus(self, amount: int, max_activations: int):
        async with self.pool.acquire() as conn:
            existing = await conn.fetchrow('SELECT * FROM daily_bonus WHERE bonus_date = CURRENT_DATE')
            if existing:
                return existing
            await conn.execute('''
                INSERT INTO daily_bonus (bonus_amount, max_activations) VALUES ($1, $2)
            ''', amount, max_activations)
            return await conn.fetchrow(
                'SELECT * FROM daily_bonus WHERE bonus_date = CURRENT_DATE ORDER BY db_id DESC LIMIT 1')
    ...
    async def claim_bonus(self, user_id: int):
        async with self.pool.acquire() as conn:
            ...
            await conn.execute(
                ...
                user_id, bonus['db_id'])
            await conn.execute(
                'UPDATE daily_bonus SET current_activations = current_activations + 1 WHERE db_id = $1',
                bonus['db_id'])
            await self.update_balance(user_id, bonus['bonus_amount'], add=True)
            updated = await conn.fetchrow('SELECT * FROM daily_bonus WHERE db_id = $1', bonus['db_id'])
            return updated, None
